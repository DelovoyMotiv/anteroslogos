    ...
    <div className="space-y-2">
      ...
      <div className="space-y-1">
        {intentTriggers.map((trigger, index) => (
          <div 
            key={index}
            className={`py-1.5 sm:py-2 px-2 bg-slate-900/50 border-l-2 ${getConfidenceColor(trigger.confidence)}`}
          >
            <div className="flex items-start justify-between gap-2 mb-1">
              <span className="font-mono text-[10px] sm:text-xs text-slate-300 uppercase">
                {trigger.intent}
              </span>
              <span className={`font-mono text-[9px] sm:text-[10px] uppercase ${getConfidenceColor(trigger.confidence)}`}>
                {trigger.confidence}
              </span>
            </div>
            
            {trigger.element.text && (
              <p className="text-[10px] sm:text-xs text-slate-500 font-mono mb-1">
                "{trigger.element.text}"
              </p>
            )}
            
            <div className="text-[9px] sm:text-[10px] text-slate-600 font-mono break-all">
              {trigger.selector}
            </div>
          </div>
        ))}
