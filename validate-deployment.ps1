# Deployment Validation Script for Anóteros Lógos
# Tests all critical SEO/GEO requirements

$baseUrl = "https://anoteroslogos.com"
$results = @()

Write-Host "`n🔍 Starting Deployment Validation..." -ForegroundColor Cyan
Write-Host "=" * 60
...
try {
    ...
    if ($response.StatusCode -eq 200) {
        Write-Host "✅ robots.txt: HTTP $($response.StatusCode)" -ForegroundColor Green
        $results += @{Test="robots.txt"; Status="PASS"}
    }
} catch {
    Write-Host "❌ robots.txt: FAILED - $_" -ForegroundColor Red
...
try {
    ...
    if ($response.StatusCode -eq 200) {
        Write-Host "✅ sitemap.xml: HTTP $($response.StatusCode)" -ForegroundColor Green
        $results += @{Test="sitemap.xml"; Status="PASS"}
    }
} catch {
    Write-Host "❌ sitemap.xml: FAILED - $_" -ForegroundColor Red
...
try {
    ...
    if ($response.StatusCode -eq 200) {
        Write-Host "✅ manifest.json: HTTP $($response.StatusCode)" -ForegroundColor Green
        $results += @{Test="manifest.json"; Status="PASS"}
    }
} catch {
    Write-Host "❌ manifest.json: FAILED - $_" -ForegroundColor Red
...
try {
    ...
    if ($response.StatusCode -eq 200) {
        Write-Host "✅ favicon.svg: HTTP $($response.StatusCode)" -ForegroundColor Green
        $results += @{Test="favicon.svg"; Status="PASS"}
    }
} catch {
    Write-Host "❌ favicon.svg: FAILED - $_" -ForegroundColor Red
    $results += @{Test="favicon.svg"; Status="FAIL"}
}

# Test 6: Check for meta tags (requires full page fetch)
...
try {
    ...
    Write-Host "✅ Meta tags found: $($foundMetas.Count)/$($requiredMetas.Count)" -ForegroundColor Green
    $results += @{Test="Meta Tags"; Status="PASS"}
} catch {
    Write-Host "❌ Meta tags: FAILED - $_" -ForegroundColor Red
...
try {
    ...
    if ($content -match 'type="application/ld\+json"') {
        ...
        Write-Host "   Found $jsonLdCount structured data blocks" -ForegroundColor Gray
        
        $results += @{Test="Schema.org"; Status="PASS"}
    } else {
        Write-Host "❌ No Schema.org structured data found" -ForegroundColor Red
        $results += @{Test="Schema.org"; Status="FAIL"}
    }
} catch {
    Write-Host "❌ Schema.org: FAILED - $_" -ForegroundColor Red
    $results += @{Test="Schema.org"; Status="FAIL"}
}
...
try {
    ...
    Write-Host "❌ SSL/HTTPS: FAILED - $_" -ForegroundColor Red
...
try {
    ...
    Write-Host "❌ Response time: FAILED - $_" -ForegroundColor Red
...
try {
    ...
    Write-Host "✅ AI crawlers configured: $($foundBots.Count)/$($requiredBots.Count)" -ForegroundColor Green
    if ($content -match "Sitemap:") {
        Write-Host "✅ Sitemap directive found in robots.txt" -ForegroundColor Green
    }
    $results += @{Test="robots.txt Content"; Status="PASS"}
} catch {
    Write-Host "❌ robots.txt content: FAILED - $_" -ForegroundColor Red
...
if ($failCount -eq 0) {
    Write-Host "ALL TESTS PASSED! Site is fully optimized for GEO." -ForegroundColor Green
    exit 0
} else {
