/**
 * Blog Data Migration Script (Admin Version)
 * Uses service role key to bypass RLS policies
 * 
 * Usage: npx tsx scripts/migrate-blog-data-admin.ts
 */
...
async function migrateBlogData(): Promise<MigrationReport> {
  console.log('🚀 Starting blog data migration (ADMIN MODE)...\n');
  console.log('⚠️  Using SERVICE ROLE KEY to bypass RLS policies\n');
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert({
          ...
          email: null,
          job_title: 'Co-founder & CEO Marketing',
          expertise: ['GEO', 'Digital Marketing', 'Brand Strategy'],
          knows_about: ['Generative Engine Optimization', 'AI Marketing', 'Digital Authority'],
        })
        .select('id, slug')
        .single();
