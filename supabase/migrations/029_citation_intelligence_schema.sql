...
-- Migration: 029_citation_intelligence_schema.sql
-- Description: Set up database schema for Predictive Citation Intelligence Engine
-- 
-- This migration creates:
-- 1. TimescaleDB extension for time series data
-- 2. Tables for citation predictions, forecasts, and temporal data
-- 3. Tables for knowledge graph storage
-- 4. Tables for ML model registry
-- 5. Tables for interventions and causal analysis
-- 6. Indexes and hypertables for performance
-- ============================================================================
...
CREATE INDEX idx_citation_predictions_user_id ON citation_predictions(user_id);
CREATE INDEX idx_citation_predictions_audit_id ON citation_predictions(audit_id);
CREATE INDEX idx_citation_predictions_url ON citation_predictions(url);
CREATE INDEX idx_citation_predictions_predicted_at ON citation_predictions(predicted_at DESC);
CREATE INDEX idx_citation_predictions_score ON citation_predictions(citation_probability DESC);

-- ============================================================================
-- Temporal Data Table (Hypertable)
...
SELECT create_hypertable('temporal_data', 'time', if_not_exists => TRUE);

-- Indexes for temporal data
CREATE INDEX idx_temporal_data_url_time ON temporal_data(url, time DESC);
CREATE INDEX idx_temporal_data_user_id ON temporal_data(user_id);
...
CREATE INDEX idx_citation_forecasts_user_id ON citation_forecasts(user_id);
CREATE INDEX idx_citation_forecasts_url ON citation_forecasts(url);
CREATE INDEX idx_citation_forecasts_forecasted_at ON citation_forecasts(forecasted_at DESC);
...
CREATE INDEX idx_kg_entities_url ON kg_entities(url);
CREATE INDEX idx_kg_entities_type ON kg_entities(entity_type);
CREATE INDEX idx_kg_entities_authority ON kg_entities(authority_score DESC NULLS LAST);
CREATE INDEX idx_kg_entities_name_search ON kg_entities USING gin(to_tsvector('english', entity_name));
...
CREATE INDEX idx_kg_relationships_user_id ON kg_relationships(user_id);
CREATE INDEX idx_kg_relationships_url ON kg_relationships(url);
...
-- Indexes for claims
CREATE INDEX idx_kg_claims_user_id ON kg_claims(user_id);
CREATE INDEX idx_kg_claims_url ON kg_claims(url);
...
CREATE INDEX idx_kg_claims_statement_search ON kg_claims USING gin(to_tsvector('english', statement));
...
CREATE INDEX idx_content_variations_url ON content_variations(url);
CREATE INDEX idx_content_variations_status ON content_variations(status);
CREATE INDEX idx_content_variations_score ON content_variations(predicted_score DESC);
...
CREATE INDEX idx_ml_models_f1_score ON ml_models(f1_score DESC NULLS LAST);
...
CREATE INDEX idx_interventions_url ON interventions(url);
CREATE INDEX idx_interventions_implemented_at ON interventions(implemented_at DESC);
...
CREATE INDEX idx_competitor_analysis_user_url ON competitor_analysis(user_url);
CREATE INDEX idx_competitor_analysis_competitor_url ON competitor_analysis(competitor_url);
...
-- TimescaleDB feature for pre-computed aggregations
-- ============================================================================

-- Daily aggregates for temporal data
CREATE MATERIALIZED VIEW IF NOT EXISTS temporal_data_daily
...
SELECT
  ...
  AVG(citation_probability) AS avg_citation_probability,
  COUNT(*) AS data_points
FROM temporal_data
GROUP BY day, url, user_id
WITH NO DATA;

-- Refresh policy for continuous aggregate
SELECT add_continuous_aggregate_policy('temporal_data_daily',
...
COMMENT ON TABLE temporal_data IS 'Time series data for historical audit scores and interventions (TimescaleDB hypertable)';
