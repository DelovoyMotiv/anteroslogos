                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        if (data.success) {
                          setCausalTrace(data.result);
                        }
                      } catch (error) {
                        console.error('Failed to load causal trace:', error);
                      } finally {
                        setIsLoadingTrace(false);
                      }
                    }}
                    disabled={isLoadingTrace}
                    className="px-4 py-2 bg-brand-accent hover:bg-blue-500 text-white rounded-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed text-sm font-semibold"
                  >
                    {isLoadingTrace ? 'Analyzing...' : 'Generate Causal Trace'}
                  </button>
                )}
              </div>
              {causalTrace ? (
                <TracerViz data={causalTrace} width={1800} height={800} />
              ) : (
                <div className="p-8 bg-white/5 border border-brand-secondary rounded-xl text-center">
                  <p className="text-white/60 mb-4">
                    Discover the exact causal path explaining why LLMs would cite your site for specific queries.
                  </p>
                  <p className="text-sm text-white/40">
                    Click "Generate Causal Trace" to visualize the decision-making process of AI platforms like Perplexity, ChatGPT, and Claude.
                  </p>
                </div>
              )}
            </div>
